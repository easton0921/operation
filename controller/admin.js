...
async function signinAdmin(req, res) {
  try {
     ...
     if(role===constants.role.ADMIN){
        ...
        }else return res.status(404).send({status:"only admin role 2 exist"})
...
async function verifySignin(req, res) {
  try {
          ...
          if (email && otp) {
            ...
            const token = jwt.genToken({ id: db._id, jti: jtiString,role:constants.role.ADMIN });
...
async function loginProfile(req, res) {
    try {
      ...
      if (email && password) {
        ...
        let token = await jwt.genToken({ id: db._id, jti: newJti,role:constants.role.ADMIN})
...
async function resetPassword(req, res) {
    try {
      ...
      let data = await User.findOne({ email: email, emailVerify: true ,role:constants.role.ADMIN})
      ...
      let data = await User.findOne({ phone: phone,countryCode:countryCode, phoneVerify: true ,role:constants.role.ADMIN})
      console.log('data', data)
      let id = data._id;
      console.timeLog("this is the id of change password of db ", id)
      const pass = await bcrypt.hashPassword(newPassword)
